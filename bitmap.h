...
#define bm_read24(__p) ({const unsigned char *_p=(__p);(*(uint16_t *)_p)|((uint32_t)(*(uint8_t *)(_p+2))<<16);})
#define bm_write24(__p,__v) ({unsigned char *_p=(__p);uint32_t _v=(__v);*(uint16_t *)_p=(uint16_t)_v;*(uint8_t *)(_p+2)=(uint8_t)(_v>>16);(void)0;})
#define bm_setpixel7(_data,_bypp,_bw,_bpp,_x,_y,_color) (\
{\
	switch((_bpp)){\
		case 8:\
...
{\
	...
	bm_setpixel7((unsigned char *)_bm+_bm->pindex,_bypp,bm_byte_width(_bm->width,_bm->bpp),_bm->bpp,(_x),(_y),(_color));\
}\
)
#define bm_getpixel6(_data,_bypp,_bw,_bpp,_x,_y) (\
{\
	uint32_t _r0;\
	switch((_bpp)){\
		case 8:\
...
{\
	const struct bitmap *restrict _bm=(__bm);\
	uint16_t _bypp=_bm->bpp/8;\
	bm_getpixel6((unsigned char *)_bm+_bm->pindex,_bypp,bm_byte_width(_bm->width,_bm->bpp),_bm->bpp,(_x),(_y));\
}\
)
#endif
